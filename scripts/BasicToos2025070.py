...
def calcDistance(curInstance_f_cd, comparedInstance_f_cd):
    distance = math.sqrt((curInstance_f_cd[4] - comparedInstance_f_cd[4])**2 + (curInstance_f_cd[5] - comparedInstance_f_cd[5])**2)
...
# ================= 多进程辅助函数 =================
# 模块级全局变量，通过 Linux fork CoW 机制在子进程间零拷贝共享
_worker_grid_instances_dic = None
_worker_instance_int_map = None
_worker_disfuncType = None
_worker_distanceThreshold = None
_worker_absolutDistance = None
_worker_wThreshold = None

def _init_worker(grid_instances_dic, instance_int_map, disfuncType, distanceThreshold, absolutDistance, wThreshold):
    """初始化子进程的全局变量"""
    global _worker_grid_instances_dic, _worker_instance_int_map
    global _worker_disfuncType, _worker_distanceThreshold, _worker_absolutDistance, _worker_wThreshold
    _worker_grid_instances_dic = grid_instances_dic
    _worker_instance_int_map = instance_int_map
    _worker_disfuncType = disfuncType
    _worker_distanceThreshold = distanceThreshold
    _worker_absolutDistance = absolutDistance
    _worker_wThreshold = wThreshold

def _process_grid_chunk(chunk_grids):
    """处理一组网格的空间距离计算，直接返回本地Pattern结果而不再保留冗长的邻居表"""
    # 内存安全阈值：累积超过此数量的键值对后自动落盘清空
    ...
    def _flush_to_disk():
        """将当前累积的数据落盘并清空内存"""
        nonlocal local_pattern_pairs, local_pattern_w_sum, total_pairs_count
        if total_pairs_count == 0:
            return
        tmp_dir = "./tmp_chunks_data"
        os.makedirs(tmp_dir, exist_ok=True)
        fd, chunk_file = tempfile.mkstemp(suffix=".pkl", dir=tmp_dir)
        with os.fdopen(fd, 'wb') as f:
            pickle.dump((local_pattern_pairs, local_pattern_w_sum), f)
        chunk_files_list.append(chunk_file)
        local_pattern_pairs = {}
        local_pattern_w_sum = {}
        total_pairs_count = 0
    
    def update_maps_new(cur_instance, compared_instance, distance, memValue):
        nonlocal total_pairs_count
        cur_int = mapInstanceToInt(cur_instance, _worker_instance_int_map)
        compared_int = mapInstanceToInt(compared_instance, _worker_instance_int_map)
        
        # cur -> compared
        pat1 = cur_instance[1] + compared_instance[1]
        if pat1 not in local_pattern_pairs:
            local_pattern_pairs[pat1] = {}
            local_pattern_w_sum[pat1] = 0.0
        if (cur_int, compared_int) not in local_pattern_pairs[pat1]:
            local_pattern_pairs[pat1][(cur_int, compared_int)] = memValue
            local_pattern_w_sum[pat1] += memValue
            total_pairs_count += 1
            
        # compared -> cur 
        pat2 = compared_instance[1] + cur_instance[1]
        if pat2 not in local_pattern_pairs:
            local_pattern_pairs[pat2] = {}
            local_pattern_w_sum[pat2] = 0.0
        if (compared_int, cur_int) not in local_pattern_pairs[pat2]:
            local_pattern_pairs[pat2][(compared_int, cur_int)] = memValue
            local_pattern_w_sum[pat2] += memValue
            total_pairs_count += 1

    directions = [(0, 1), (1, 1), (0, 0), (1, 0), (1, -1)]
    for grid_position in chunk_grids:
        cur_instances = _worker_grid_instances_dic.get(grid_position, [])
        for cur_instance in cur_instances:
            for dx, dy in directions:
                compared_grid_position = (grid_position[0] + dx, grid_position[1] + dy)
                if compared_grid_position in _worker_grid_instances_dic:
                    for compared_instance in _worker_grid_instances_dic[compared_grid_position]:
                        if _worker_disfuncType == 0:
                            memValue, distance = getInstancePairW(cur_instance, compared_instance, _worker_distanceThreshold)
                        elif _worker_disfuncType == 1:
                            memValue = calcDistance(cur_instance, compared_instance)
                            distance = memValue
                        elif _worker_disfuncType == 2:
                            memValue, distance = calcMemValue(cur_instance, compared_instance, _worker_absolutDistance, _worker_distanceThreshold)
                            
                        if ((_worker_disfuncType == 1 and memValue < _worker_wThreshold) or
                            (_worker_disfuncType == 2 and memValue > _worker_wThreshold) or
                            (_worker_disfuncType == 0 and distance < _worker_wThreshold)):
                            
                            if cur_instance[1] != compared_instance[1]:
                                update_maps_new(cur_instance, compared_instance, distance, memValue)
        
        # 每处理完一个网格后检查是否需要落盘
        if total_pairs_count >= FLUSH_THRESHOLD:
            _flush_to_disk()
    
    # 将剩余数据落盘
    _flush_to_disk()
        
    return chunk_files_list, len(chunk_grids)
# ===================================================
...
def getStarNei(filePath,absolutDistance, distanceThreshold, wThreshold_f_gsn,disfuncType):
    ...
    print("载入实例的个数为",len(instances_f_gsn), flush=True)
    ...
    for instance in instances_f_gsn:
        ...
        if grid_position in grid_instances_dic:
    ...
    total_grids = len(grid_instances_dic)
    processed_grids = 0
    print(f"[*] 开始计算全域 {total_grids} 个网格矩阵的高斯空间距离(23亿次级求幂)...", flush=True)
    
    n_workers = int(os.environ.get("N_WORKERS", max(1, os.cpu_count() - 1)))
    print(f"   [模式] 多进程并行预计算 (Workers={n_workers})", flush=True)
    
    all_grids = list(grid_instances_dic.keys())
    chunk_size = 20  # 直接强制缩小每个批次的网格包含数量
    chunks = [all_grids[i:i + chunk_size] for i in range(0, len(all_grids), chunk_size)]
    total_chunks = len(chunks)
    completed_chunks = 0
    
    chunk_files_returned = []
    
    # 使用 initializer 在每个子进程中初始化全局变量（兼容 Windows spawn 模式）
    with multiprocessing.Pool(
        processes=n_workers, 
        maxtasksperchild=1,
        initializer=_init_worker,
        initargs=(grid_instances_dic, instance_int_map_f_gsn, disfuncType, distanceThreshold, absolutDistance, wThreshold_f_gsn)
    ) as pool:
        for chunk_files, num_grids in pool.imap_unordered(_process_grid_chunk, chunks):
            processed_grids += num_grids
            completed_chunks += 1
            print(f"   --> 已处理网格进度: {processed_grids}/{total_grids} ({processed_grids/total_grids*100:.1f}%) [chunk {completed_chunks}/{total_chunks}] 碎片文件: {len(chunk_files)}", flush=True)
            
            chunk_files_returned.extend(chunk_files)
            
    print("网格空间距离计算完成", flush=True)
    return intInstanceMap_f_gsn, instance_int_map_f_gsn, eventCount_f_gsn, Event_instanceMap, chunk_files_returned, None
...
#生成二阶表实例
#输入：cnmMap_f_gl2ti 中心实例和邻居实例的字典 (兼容 IPC 管廊 MapReduce 化重构，当前传入的为 chunk_files 列表 )
...
def genLevel2TableInstance(intInstanceMap_f_gl2ti, cnmMap_f_gl2ti):
    if isinstance(cnmMap_f_gl2ti, list):
        chunk_files = cnmMap_f_gl2ti
        
        # 1. 扫描并增量累加计算 FPI (由于去掉了 global_w_sum，现在我们直接在汇总阶段扫描文件统计 w_sum)
        patternInsWSum_f_gl2ti = {}
        for file in chunk_files:
            try:
                with open(file, 'rb') as f:
                    _, local_w_sum = pickle.load(f)
                for pat, w in local_w_sum.items():
                    patternInsWSum_f_gl2ti[pat] = patternInsWSum_f_gl2ti.get(pat, 0.0) + w
            except Exception as e:
                pass
                
        # 计算评分 FPI 
        patternFpi_f_gl2ti = {}
        for pattern_f_cf, w_sum in patternInsWSum_f_gl2ti.items():
            maxNumOfEventCount_f_cf = 0
            for i_f_cf in pattern_f_cf:
                maxNumOfEventCount_f_cf = max(maxNumOfEventCount_f_cf, eventCount.get(i_f_cf, 1))
            if maxNumOfEventCount_f_cf > 0:
                patternFpi_f_gl2ti[pattern_f_cf] = round(w_sum/pow(maxNumOfEventCount_f_cf,2),6)
                
        # 2. 从碎片文件合并实例对，只取包含实例去重的集合以大幅压缩内存
        ...
        for file in chunk_files:
            try:
                ...
                for pat, pairs_dict in local_pairs.items():
                    if pat not in patternTableInstance_f_gl2ti:
                        patternTableInstance_f_gl2ti[pat] = set()
                    # 兼容：落盘现在是一个带有 memValue 的字典，我们只需提取 keys() 放进集合
                    patternTableInstance_f_gl2ti[pat].update(pairs_dict.keys())
            except Exception as e:
                pass
            finally:
                # 保留这部分切片文件供 FPI_joinless 第二阶段流式读取，因此去掉旧版的直接删除落盘文件操作
                pass
                    
        # 3. List 化规整返回值格式 
        for pat in patternTableInstance_f_gl2ti:
            patternTableInstance_f_gl2ti[pat] = list(patternTableInstance_f_gl2ti[pat])
...
def getInstancePairW(instanceA_f_gipw, instanceB_f_gipw, distanceThreshold):
    dis = math.sqrt((instanceA_f_gipw[4] - instanceB_f_gipw[4])**2 + (instanceA_f_gipw[5] - instanceB_f_gipw[5])**2)
    w_f_gipw = math.exp(-(dis**2)/(2 * distanceThreshold**2))
    return  w_f_gipw, dis
...
if __name__ == "__main__":
    # 修正：依据三十万昆明全集真实运算均值，FPI 核心商业聚集阈值应落在千分位，设定 0.001 即可挑出最具价值的前三十套组合
    fpiThreshold = 0.001
    distanceThreshold = 5
    wThreshold = 0.5

    # 改造点1：替换数据源路径，读取同目录网格数据
    print("[-] 开始读取全域数据，过程可能持续数分钟，请耐心等待...", flush=True)
    idInstanceMap, instanceIdMap, eventCount, Event_instanceMap, centerIndex_neiIndex_memValueMap, patternDisMap = getStarNei("./kunming_poi.data",20,distanceThreshold,distanceThreshold,0)
    #生成二阶表实例
    patternPiMap, patternTableInstance = genLevel2TableInstance(idInstanceMap,centerIndex_neiIndex_memValueMap)
    
    # ================= 改造核心区：生成适配大屏的 CSV 与 JSON =================
    # 改造点2：过滤高频模式并输出标准格式的FPI榜单 CSV
    filtered_patterns = []
    pattern_id_counter = 1
    for pattern, fpi in patternPiMap.items():
        if fpi >= fpiThreshold:  
            filtered_patterns.append({
                "pattern_id": pattern_id_counter,
                "pattern_name": pattern,
                "fpi_score": round(fpi, 6)
            })
            pattern_id_counter += 1
            
    filtered_patterns.sort(key=lambda x: x["fpi_score"], reverse=True)
    fpi_df = pd.DataFrame(filtered_patterns)
    fpi_df.to_csv("./fpi_patterns.csv", index=False)
    print(f"[+] 已生成大屏需要的 FPI 排行榜单 fpi_patterns.csv，共挖掘出 {len(filtered_patterns)} 个频繁模式。")

    # 改造点3：全场最核心的“埋点”——提取空间团伙 ID 转化为 json
    import json
    pattern_instances_dict = {}
    name_to_id = {item["pattern_name"]: item["pattern_id"] for item in filtered_patterns}
    
    for pattern_name, instances_pairs in patternTableInstance.items():
        if pattern_name in name_to_id: # 只存上了榜单的模式的实例
            p_id = name_to_id[pattern_name]
            poi_id_set = set() # 用 set 去重
            for pair in instances_pairs:
                index1, index2 = pair
                # 通过 idInstanceMap 获取真实的 poi_id (元组的第0个元素)
                poi_id1 = idInstanceMap[index1][0]
                poi_id2 = idInstanceMap[index2][0]
                poi_id_set.add(int(poi_id1))
                poi_id_set.add(int(poi_id2))
            
            pattern_instances_dict[str(p_id)] = {
                "pattern_name": pattern_name,
                "poi_ids_list": list(poi_id_set)
            }
            
    with open("./pattern_instances.json", "w", encoding="utf-8") as f:
        json.dump(pattern_instances_dict, f, ensure_ascii=False, indent=2)
    print("[+] 已成功提取大屏前端绘图所必须的坐标实例对照表 pattern_instances.json。")
    # =========================================================================
    ...
    # 为保障服务器端数据流转畅通，在大屏环境运行时关闭本地 Matplotlib 弹窗
    # plt.show()
